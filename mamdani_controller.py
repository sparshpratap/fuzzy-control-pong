...
class FuzzyPlayer(Player):
    ...
    def __init__(self, racket: Racket, ball: Ball, board: Board):
        ...
        self.max_move = racket.max_speed * 2.0  

        width = board.surface.get_width()
        self.x_universe = np.linspace(-width / 2, width / 2, 801)
        self.vel_universe = np.linspace(-self.max_move, self.max_move, 401)

        self.x_mf = {
            "far_left":  fuzz.trapmf(self.x_universe, [-width/2, -width/2, -250, -120]),
            ...
            "far_right": fuzz.trapmf(self.x_universe, [120, 250, width/2, width/2]),
        }

        self.vel_mf = {
            "far_left":  fuzz.trimf(self.vel_universe, [-self.max_move, -racket.max_speed, -racket.max_speed*0.5]),
            "left":      fuzz.trimf(self.vel_universe, [-racket.max_speed, -racket.max_speed*0.5, 0]),
            "center":    fuzz.trimf(self.vel_universe, [-0.5, 0, 0.5]),
            "right":     fuzz.trimf(self.vel_universe, [0, racket.max_speed*0.5, racket.max_speed]),
            "far_right": fuzz.trimf(self.vel_universe, [racket.max_speed*0.5, racket.max_speed, self.max_move]),
        }

    def act(self, x_diff, y_diff):
        error = -x_diff

        if abs(error) < self.racket.width // 2:
            target_x = self.ball.rect.centerx - self.racket.width // 2
            self.racket.move(target_x, self.board)
            return

        move_x = self._mam_move(error)
        self.racket.move(self.racket.rect.x + move_x, self.board)

    def _mam_move(self, error):
        mem = {k: fuzz.interp_membership(self.x_universe, mf, error)
               for k, mf in self.x_mf.items()}

        agg = np.zeros_like(self.vel_universe)
        for k, deg in mem.items():
            if deg > 0:
                agg = np.fmax(agg, np.fmin(deg, self.vel_mf[k]))

        dx = fuzz.defuzz(self.vel_universe, agg, 'centroid')
