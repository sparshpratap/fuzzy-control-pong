...
class FuzzyPlayer(Player):
    def __init__(self, racket: Racket, ball: Ball, board: Board):
        ...
        self.vel_fx = {
            "far_left":  lambda e, xs, ys: -(abs(e) * 0.4 + 8) - xs * 0.5,
            "left":      lambda e, xs, ys: -(abs(e) * 0.2 + 4) - xs * 0.3,
            "center":    lambda e, xs, ys: e * 0.1 - xs * 0.05,
            "right":     lambda e, xs, ys: (abs(e) * 0.2 + 4) - xs * 0.3,
            "far_right": lambda e, xs, ys: (abs(e) * 0.4 + 8) - xs * 0.5,
        }

        self.max_move = racket.max_speed * 1.5
        #self._visualize_mfs()

    def _visualize_mfs(self):
        plt.figure(figsize=(7, 4))
        for name, mf in self.x_mf.items():
            plt.plot(self.x_universe, mf, label=name)
        plt.title("Fuzzy Membership Functions for Horizontal Error")
        plt.xlabel("Error (ball_x - paddle_x)")
        plt.ylabel("Membership Degree (µ)")
        plt.legend()
    ...
    def act(self, x_diff, y_diff):
        error = -x_diff
        xs, ys = self.ball.x_speed, self.ball.y_speed
        dx = self._tsk_velocity(error, xs, ys)
        self.racket.move(self.racket.rect.x + dx, self.board)

    def _tsk_velocity(self, error, xs, ys):
        memberships = {
            name: fuzz.interp_membership(self.x_universe, mf, error)
            for name, mf in self.x_mf.items()
        }
        
        numerator, denominator = 0.0, 0.0
        for name, mu in memberships.items():
            if mu <= 0:
                continue
            output = self.vel_fx[name](error, xs, ys)
            numerator += mu * output
            denominator += mu

        dx = numerator / denominator if denominator else 0

        return max(-self.max_move, min(self.max_move, dx))
